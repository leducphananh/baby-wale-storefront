...
| `/dat-hang-thanh-cong/[token]` | Confirm receipt; give payment instructions & tracking | "Did it go through? How do I pay? How do I check later?" | **Theo dõi đơn hàng** (this URL) | Copy/save the link; read payment instructions; contact support; continue shopping | Confirmation heading; order code; customer-facing status; payment instructions (method-specific); order summary; tracking link + "save this link"; support contact | loading; found; token invalid / not found; (bank-transfer vs COD variants) | Payment instructions prominent for bank transfer; link easy to copy | **`noindex`** + disallow. Rendered `no-store`. |
...
6. **Tracking link** — this same URL; a "Lưu lại đường dẫn này để theo dõi đơn hàng"
   hint (+ copy affordance). Since there is no account, the link is the customer's only
   handle.
7. **Contact support** — phone / Zalo.
8. **"Tiếp tục mua sắm"** (→ Home).
...
For customers who saved the link but not the page. Manual **token** entry only.

| State | Behavior |
| --- | --- |
| idle | One field: "Nhập mã theo dõi đơn hàng" (from the link the customer saved) + short help text. Submit: **"Tra cứu"**. |
...
| Internal (`orders.status`, `source='website'`) | S0 wording | **S2.1 recommendation** | Rationale |
| --- | --- | --- | --- |
| `draft` | "Đơn mới – chờ xác nhận" | **"Đơn mới – chờ xác nhận"** (keep) | Clear; sets the right expectation (staff will confirm). |
| `completed` | "Đã xử lý / hoàn tất" | **"Đơn hàng đã được xác nhận"** | In MVP there is no delivery-tracking state; admin `complete_order` means *accepted / stock allocated / now real*, **not** *delivered*. "hoàn tất / completed" reads as "delivered/done" to a customer and over-promises. "Đã được xác nhận" is honest. *(Owner decision — §25; DB enums unchanged.)* |
...
### 15.1 Sticky surfaces — recommendation (max one per screen)

| Surface | Mobile | Desktop | Verdict |
| --- | --- | --- | --- |
| Product detail — Add-to-Cart bar | **Yes** — appears after the in-flow purchase block scrolls away; shows price + quantity + "Thêm vào giỏ" (or disabled "Hết hàng"). | Purchase panel **sticks within the viewport** as gallery/description scroll. | **Useful — implement.** |
| Cart page — checkout CTA | **Yes** — sticky bar with merchandise subtotal + "Tiến hành đặt hàng" (long lists make the bottom CTA far away). | Not needed (side panel is visible). | **Useful on mobile.** |
| Checkout — order summary | **No** — collapsed summary at top + full summary above submit; a sticky bar competes with the keyboard and the submit. | **Yes** — sticky right aside. | **Desktop only.** |
| Header | Sticky (compact) so search + cart are always reachable. | Sticky optional. | Keep the sticky header short. |

Do **not** stack sticky bars. Do **not** make filters or breadcrumbs sticky.
...
| Order success | **Theo dõi đơn hàng** | Tiếp tục mua sắm; (payment instructions are prominent content, not a CTA) |
...
### D1 — Customer label for a `completed` website order

- **Recommendation:** use **"Đơn hàng đã được xác nhận"** (not the S0 draft wording
  "Đã xử lý / hoàn tất").
...
### D2 — Direct "Thêm vào giỏ" on product listing cards

- **Recommendation:** **not in MVP.** Cards link to detail; adding happens on the
  product page.
...
### D3 — Home page: static value block vs rotating hero/banner

- **Recommendation:** **static value block** for MVP — brand line + prominent search +
...
### D4 — Is there a returns/exchange policy to publish at all?
...
- **Recommendation:** if **yes**, reserve a footer "Chính sách" section and a short
  ...
  without it — **do not fabricate one**.
