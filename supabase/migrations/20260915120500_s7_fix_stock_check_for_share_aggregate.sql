-- Follow-up to 20260915120000_s7_storefront_order_backend_contract: real
-- black-box testing against the live project (anon, via PostgREST) caught
-- a genuine Postgres error the first version of create_storefront_order()
-- could never have hit in a purely code-reviewed check: `FOR SHARE is not
-- allowed with aggregate functions`. The stock-check query combined
-- `sum(pb.remaining_quantity)` with `FOR SHARE` in a single statement,
-- which Postgres rejects outright (row locking cannot be expressed
-- directly alongside aggregation in the same SELECT). Every real checkout
-- attempt failed with ORDER_CREATE_FAILED-shaped noise before this fix.
--
-- Fix: lock the candidate batch rows first (a plain, non-aggregate SELECT
-- ... FOR SHARE), then aggregate over that locked set in a wrapping
-- subquery. Locking semantics are unchanged from the original design intent
-- (S0 §37.5 — "a check, not a lock", reducing but not eliminating a stock
-- race against a concurrent checkout of the same product) — only the SQL
-- shape needed correcting. No other part of the function changes.
