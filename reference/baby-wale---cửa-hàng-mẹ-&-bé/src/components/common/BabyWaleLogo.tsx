...
export const BabyWaleLogo: React.FC<BabyWaleLogoProps> = ({
  size = 'md',
  showText = true,
  className = '',
  onClick,
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={`inline-flex items-center gap-2.5 select-none transition-transform duration-200 active:scale-95 ${
      ...
      {showText && (
        <div className="flex flex-col">
          <div className="flex items-center gap-1">
            <span
              className={`font-extrabold tracking-tight text-primary leading-none ${dimensions.title}`}
            >
              Baby Wale
            </span>
            <span className="w-1.5 h-1.5 rounded-full bg-secondary inline-block mb-1"></span>
          </div>
          <span className={`text-muted-foreground font-semibold tracking-wide ${dimensions.sub}`}>
