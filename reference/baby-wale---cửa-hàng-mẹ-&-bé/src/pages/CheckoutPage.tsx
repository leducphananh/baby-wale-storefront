...
export const CheckoutPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState<ShippingAddress>({
    ...
    ward: currentUser?.addresses?.[0]?.ward || '',
    district: currentUser?.addresses?.[0]?.district || '',
    city: currentUser?.addresses?.[0]?.city || 'Hồ Chí Minh',
    notes: '',
  });
  ...
  // Shipping cost adjustment for Express
  ...
  const handleOrderSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) {
      addToast('Thông tin chưa đầy đủ', 'Vui lòng kiểm tra lại các trường bắt buộc', 'warning');
      return;
    }
    ...
    setTimeout(() => {
      const order = placeOrder({
        ...
        paymentMethod,
        shippingMethod: shippingMethod === 'express' ? 'Giao hỏa tốc 2H' : 'Giao hàng tiêu chuẩn',
      });
  ...
  return (
    <div id="baby-wale-checkout-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      {/* Breadcrumbs */}
      <Breadcrumbs items={[{ label: 'Giỏ hàng', page: 'cart' }, { label: 'Thanh toán an toàn' }]} />
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Quét mã QR qua ứng dụng ngân hàng, xác nhận tức thì không mất phí.
                  </p>
                </div>
              </label>
              ...
              >
                <Smartphone className="w-5 h-5 text-pink-600 shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-bold text-xs sm:text-sm text-foreground">Ví điện tử MoMo</h4>
                  <p className="text-[11px] text-muted-foreground">
                    Thanh toán nhanh chóng và tích điểm qua ứng dụng MoMo.
                  </p>
                </div>
              </label>
            </div>
                ...
                <div key={item.id} className="flex items-center gap-3">
                  <div className="relative">
                    <img
                      ...
                      className="w-12 h-12 rounded-xl object-contain bg-muted p-1 border border-border"
            ...
            <div className="pt-3 border-t border-border space-y-2 text-xs text-muted-foreground">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-foreground">
                  {effectiveShipping === 0 ? 'Miễn phí' : formatVND(effectiveShipping)}
                </span>
              </div>
              {couponDiscount > 0 && (
