...
export const ContactPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.fullName.trim() || !formData.phoneNumber.trim() || !formData.message.trim()) {
      addToast('Thiếu thông tin', 'Vui lòng điền họ tên, số điện thoại và nội dung cần nhắn', 'warning');
      return;
    }
    addToast(
      'Gửi tin nhắn thành công!',
      'Chuyên viên tư vấn của Baby Wale sẽ liên hệ lại với bạn trong vòng 30 phút.',
      'success'
    );
    setFormData({
      fullName: '',
  ...
  return (
    <div id="baby-wale-contact-page" className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-12">
      <Breadcrumbs items={[{ label: 'Liên hệ & Cửa hàng' }]} />
          ...
          {stores.map((store, idx) => (
            ...
            >
              <div className="space-y-2.5">
                <h3 className="font-bold text-sm text-foreground text-primary">{store.name}</h3>
